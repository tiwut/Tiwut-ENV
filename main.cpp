...
static void server_new_xdg_surface(wl_listener *listener, void *data) {
  ...
  if (xdg_surface->role == WLR_XDG_SURFACE_ROLE_TOPLEVEL) {
    ...
    view->scene_tree =
        wlr_scene_xdg_surface_create(server->scene_layout, xdg_surface);
...
static void output_frame(wl_listener *listener, void *data) {
  ...
  wlr_scene_output *scene_output =
      wlr_scene_get_scene_output(scene, output->wlr_out);
...
int main() {
  ...
  if (fork() == 0) {
    ...
    std::string cmd = "chromium --kiosk --app=file://" + std::string(ui_path) +
                      "/ui/index.html &";
